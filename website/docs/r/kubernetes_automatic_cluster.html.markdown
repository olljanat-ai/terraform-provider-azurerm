...
* `default_nginx_controller` - (Optional) Specifies the ingress type for the default `NginxIngressController` custom resource. The allowed values are `AnnotationControlled`, `External`, `Internal` and `None`. At least one of `default_nginx_controller` or `istio_enabled` must be specified.
